 ...
 * timer interrupt. 
 */
#define TIMER_INTERRUPTS_PER_SECOND 10
 ...
 */
#define FW_VERSION_MAJOR ((uint8_t)0x05)
...
//Algo bits:
#define WATER_BAD_SAMPLE      BIT0
#define WATER_VOLUME_CAPPED   BIT1
#define WATER_STANDING        BIT2

//app level bits:
#define NO_RTC_TIME           BIT3
...
typedef struct sysExecData_s {
    ...
    uint16_t dry_wake_time;                                /**< Number of consecutive trends with no water before sleeping - not used.*/
...
typedef struct storageData_s {
    ...
    uint16_t minuteMilliliterSum;                          /**< Running milliliter sum for current minute - not used */
...
void msgSched_scheduleFinalAssemblyMessage(void);
#ifdef SEND_DEBUG_TIME_DATA
