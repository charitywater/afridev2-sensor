...
void modemMgr_init(void)
{
    ...
    // clear out old OTA messages
    memset(&mwBatchData, 0, sizeof(mwBatchData_t));
    memset(otaBuf, 0, OTA_PAYLOAD_MAX_RX_READ_LENGTH);
...
void modemMgr_sendModemCmdBatch(modemCmdWriteData_t *cmdWriteP)
{
    ...
    if (cmdWriteP->cmd == OUTPOUR_M_COMMAND_SEND_TEST)
    {
        mwBatchData.sendTestActive = SEND_TEST_RETRIES;
        sysExec_set_send_test_result(SYSEXEC_SEND_TEST_RUNNING);
    }
    else
    {
        // no special MODEM STATUS waiting for any other message
        mwBatchData.sendTestActive = 0;
    }
        ...
        switch (mwBatchData.mwBatchState)
        {
            ...
            case MWBATCH_STATE_MODEM_STATUS_WAIT:
                if (!modemCmd_isBusy())
                {
                    ...
                    if (mwBatchData.sendTestActive > 0)
                    {
                        // if transmission in progress, wait at least 1 second and resend MODEM_STATUS
                        if (!modemMgr_isLinkUp() && !modemMgr_isLinkUpError())
                        {
                            // come back in a second and try for status again
                            mwBatchData.mwBatchState = MWBATCH_STATE_MODEM_STATUS;
                            mwBatchData.sendTestActive--;
                            WATCHDOG_TICKLE();
                        }
                        else
                        {
                            // isLinkUp indicates a "Success" response.. we are done
                            if (modemMgr_isLinkUp())
                                sysExec_set_send_test_result(SYSEXEC_SEND_TEST_PASS);
                            else
                                sysExec_set_send_test_result(SYSEXEC_SEND_TEST_FAIL);
                            // Next state
